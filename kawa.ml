...
type expr =
  ...
  (*Creation tableau*)
  | CreationTab of typ * expr list 
  (*Creation d'un tableau initialisée*)
  | CreationTabInit of expr list

(* Accès mémoire : variable ou attribut d'un objet *)
and mem_access =
  | Var   of string
  | Field of expr (* objet *) * string (* nom d'un attribut *)
  | ArrayAcces of mem_access * expr 
...
type class_def = {
    class_name: string;
    attributes: (string * (expr option) * typ ) list;
